 ...
 * @author ADMIN
 */
public class QuestionListController extends BaseAuthController {
    ...
    protected void processGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int coursesid = -1;
        if (courseid != null) {
            coursesid = Integer.parseInt(courseid);
        }
        ...
        String quizlevels = request.getParameter("quizid");
        int qid = -1;
        if (quizlevels != null) {
            qid = Integer.parseInt(quizlevels);
        }

//      
        QuestionDBContext q = new QuestionDBContext();
        ArrayList<Question> question = q.getFilterQuestion(stt, did, qid, coursesid);
        request.setAttribute("list", question);
//        
        request.setAttribute("coursesid", coursesid);
...
//      
        QuestionDBContext q = new QuestionDBContext();
        ArrayList<Question> question = q.getFilterQuestion(stt, did, qid, coursesid);
        request.getSession().setAttribute("list", question);

//       
//        request.setAttribute("qid", qid);
//        request.setAttribute("stt", stt);
//        request.setAttribute("did", did);
response.getWriter().print("<table id=\"datatablesSimple\"  >\n"
                    + "                                        <thead>\n"
                    + "                                            <tr>\n"
                    + "                                                <th>ID</th>\n"
                    + "                                                <th>Content</th>   \n"
                    ...
                    + "                                        <tbody >\n"
                    + "\n");
        for (Question qe : question) {
            response.getWriter().print("<tr>\n"
                    + "                                                <td>000" + qe.getId() + "</td>\n"
                    + "\n"
                    + "                                                <td><div style=\"word-wrap: break-word\">" + qe.getContent() + " </div>\n"
                    + "                                                </td>\n"
                    + "\n"
                    + "                                                <td>" + qe.getAnswer() + "</td>     \n"
                    + "                                                <!--<td>" + qe.getExplain() + "</td>-->\n"
                    + "\n"
                    + "                                                <td>" + qe.getCourse().getTitle() + "</td>\n"
                    + "                                                <td>" + qe.getQuizlevel().getLevelName() + "</td>\n"
                    + "\n"
