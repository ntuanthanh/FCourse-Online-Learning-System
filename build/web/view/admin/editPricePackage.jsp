...
<!--        <link rel="stylesheet" type="text/css" href="/summer2022-se1616-g4/view/admin/assets/css/shortcodes/shortcodes.css">-->
    ...
    <body class="ttr-opened-sidebar ttr-pinned-sidebar">
        ...
        <div class="container mt-3">
            <h2 style="border-bottom: 1px solid black; color: #F68051">Edit Price Package</h2>
            <br>
            ...
            <div class = "container-edit-pricepackage">
                <form id = "validate-form-edit-pricepackage" action = "edit"  method="post">
                    <div class = "row">
                          <div class="form-group col-md-12">
                            <input name ="cid" type="text" value="${requestScope.cid}" hidden="hidden" >
                            <input name ="pid" type="text" value="${requestScope.pricepackage.id}" hidden="hidden" >
                            <label for = "pname" class="col-form-label font-weight-subject-overview">Price Package Name</label>
                            <div>
                                <input id = "pname" name ="pname" class="form-control" type="text" value="${requestScope.pricepackage.name}" placeholder="Enter package name..." >
                            </div>
                          </div>
                          <div class="form-group col-md-12">
                            <label for = "duration" class="col-form-label font-weight-subject-overview">Duration ( month )</label>
                            <div>
                                <input id = "duration" name ="duration" class="form-control" type="text" value="${requestScope.pricepackage.duration}" placeholder="Enter number month...">
                            </div>
                          </div>
                            <div class="form-group col-md-12">
                            <label for = "listprice" class="col-form-label font-weight-subject-overview">List Price ($)</label>
                            <div>
                                <input id = "listprice" name ="listprice" class="form-control" type="text" value="${requestScope.pricepackage.listPrice}" placeholder="Enter list price...">
                            </div>
                          </div>
                          <div class="form-group col-md-12">
                            <label for = "saleprice" class="col-form-label font-weight-subject-overview">Sale Price ($)</label>
                            <div>
                                <input id = "saleprice" name ="saleprice" class="form-control" type="text" value="${requestScope.pricepackage.salePrice}" placeholder="Enter sale price...">
                            </div>
                          </div>
                          <div class = "status_pricepackage">
                            ...
                            <select style="text-transform: capitalize" id="status_id" name = "status_id">
                                <c:forEach items="${requestScope.statuses}" var ="s">
                                    <option value ="${s.id}" ${s.id == requestScope.pricepackage.status.id?"selected = selected":""} >${s.name}</option>
                                </c:forEach>
                            </select>
                          </div>    
        ...
        <script>
            <c:if test="${sessionScope.editPricePackage_mess!=null}">
                Swal.fire(
                   'Update successfully',
                   'Update Price Package successfully ',
                    'success'
                         )
                <% request.getSession().setAttribute("editPricePackage_mess", null); %>
            </c:if>
//            if(${requestScope.mess != null}){          
//                Swal.fire(
//                    'Update successfully',
//                    'Update Dimension successfully ',
//                    'success'
//                          )
//            }
